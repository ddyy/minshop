...
- **Shipping labels (Shippo)** — optional: add a Shippo API token in **Admin → Settings** and a **Buy shipping label** card appears on unfulfilled orders, prefilled with the order's recorded address and shipment weight. Buying a label records the tracking number, marks the order fulfilled, emails the customer, and keeps the label PDF link on the order. A `shippo_test_…` token purchases fake labels for trying the flow. Rates at checkout still come from your configured zones — the carrier only enters at fulfillment.
